...
RC Test3 (void)
{
    ...
    PF_Manager pfm = PF_Manager ();  
    IX_Manager ixm = IX_Manager (pfm);
    RM_Manager rmm = RM_Manager (pfm); 
    SM_Manager manager = SM_Manager (ixm, rmm); 
    manager.OpenDb ("red"); 
    AttrInfo attributes[3];
    attributes[0].attrName = "string"; 
    attributes[0].attrType = STRING;
    attributes[0].attrLength = 32;
    attributes[1].attrName = "int";
    attributes[1].attrType = INT;
    attributes[1].attrLength = 4; 
    attributes[2].attrName = "float";
    attributes[2].attrType = FLOAT;
    attributes[2].attrLength = 4; 
    manager.CreateTable (FILENAME, 3, attributes); 
    //manager.CreateIndex (FILENAME, "string"); 
    //manager.CreateIndex (FILENAME, "int"); 
    //manager.CreateIndex (FILENAME, "float"); 
    //manager.DropIndex (FILENAME, "string"); 
    //manager.DropIndex (FILENAME, "int"); 
    //manager.DropIndex (FILENAME, "float");
    manager.Print (FILENAME); 
    manager.Help (FILENAME); 
    manager.DropTable (FILENAME);  
    manager.Help (FILENAME); 
    manager.CloseDb ();
